...
- **kling_elements**: Optional facial consistency element. Only available when `character_orientation=video`; forwarded to the upstream `elements` field
...
- `kling_elements` supports only `1` facial element. Each element requires `frontal_image_url` and `reference_image_urls` with `1` to `3` images
- When using `kling_elements`, reference the element in your prompt as `@Element1`
