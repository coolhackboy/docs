...
| â†?| `audio_url` | string | Direct download URL for the audio file |
| â†?| `image_url` | string | Cover image URL |
| â†?| `title` | string | Track title |
| â†?| `tags` | string | Style tags |
| â†?| `duration` | number | Audio duration in seconds |
| â†?| `prompt` | string | Generation prompt used |
| `get-timestamped-lyrics` | `timestampe_lyrics` | string | Lyrics with timestamps |
| `generate-lyrics` | `title` | string | Lyrics title |
| â†?| `text` | string | Complete lyrics content |
