...
| `stem-split-advanced` | `origin_data` | array | Selected stem results with `extract` and `remove` objects |
...
### Advanced Stem result example

```json
{"task_id":"POYO_STEM_TASK","status":"finished","credits_amount":20,"files":[{"origin_data":[{"extract":{"stem_type_group_name":"Guitar","duration":194.92,"audio_url":"https://storage.poyo.ai/extract.mp3"},"remove":{"stem_type_group_name":"Guitar","duration":194.92,"audio_url":"https://storage.poyo.ai/remove.mp3"}}]}],"error_message":null}
```
