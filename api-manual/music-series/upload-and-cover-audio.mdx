...
Upload Cover supports `persona_model` (`style_persona` or `voice_persona`) with `persona_id` only in Custom Mode on V5/V5_5. It also supports integer `duration` from 10–360 only when `custom_mode: true` and `mv: V5_5`.
...
- `persona_id` (string): Persona ID to apply to the covered track. Only available when `custom_mode: true` with V5/V5_5. How to generate persona_id, visit [generate-persona](/api-manual/music-series/generate-persona).

- `persona_model` (string): `style_persona` or `voice_persona`. Requires `persona_id`, `custom_mode: true`, and V5/V5_5.
