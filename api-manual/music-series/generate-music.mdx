...
- In Custom Mode ( `custom_mode: true` ):
  ...
  - Character limits vary by model:
    ...
    - **V5_5**: `prompt` 5000 characters, `style` 1000 characters
...
- `persona_model` (string): `style_persona` or `voice_persona`. Requires `persona_id` and V5/V5_5.

- `duration` (integer): Custom length from 10–360 seconds. Only supported when `custom_mode: true` and `mv: V5_5`.
