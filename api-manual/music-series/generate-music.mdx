...
- In Custom Mode ( `custom_mode: true` ):
  ...
  - If `instrumental: false` : `style` and `title` are required
...
- `persona_id` (string): Persona ID to apply to the generated music. Use this to apply a specific persona style to your music generation. Only available when Custom Mode is enabled.
