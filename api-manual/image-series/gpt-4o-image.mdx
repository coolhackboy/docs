...
| `input.prompt` | string | Description of the desired image (max 1000 characters) |
...
| `input.size` | string | `` | Image dimensions. Aspect ratio: `1:1`, `2:3`, `3:2` |
| `input.n` | integer | `1` | Number of images to generate. Supports `1`, `2`, or `4` |
| `input.image_urls` | array | - | Reference image URLs for image-to-image or editing (max 5 images) |
| `input.mask_url` | string | - | PNG format mask URL for editing (required for `gpt-4o-image-edit`) |
...
| `data.files` | array | Array of generated image files |
| `data.files[].file_url` | string | Direct URL to generated file (valid for 24 hours) |
| `data.files[].file_type` | string | Type of media file: `image` |
| `data.created_time` | string | ISO 8601 timestamp of task creation |
| `data.progress` | integer | Completion percentage (100 when finished) |
| `data.error_message` | string | Error description (null when successful) |
...
```json
{
  "code": 400,
  "error": {
    "message": "Invalid parameter: size must be one of 1:1, 2:3, 3:2",
