...
The public Capability Catalog and MCP discovery tools do not require authentication. A successful model search therefore does not confirm that the configured API key is valid; use `poyo_account` or `poyo auth status` to verify authentication.
