...
The examples below use the shorter `poyo` command. Install it globally first, or replace `poyo` with `npx -y @poyo/cli`:

```bash
npm install --global @poyo/cli
```
...
poyo mcp --models MODEL_A,MODEL_B
```
